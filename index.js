...
function getArtistByIndex(artistsArray, index) {
	const { id, name } = artistsArray[index];
...
function get20s(artistsArray) {
	...
	for (let i = 0; i < artistsArray.length; i++) {
		// Setting up artist data for readability
		const currentArtist = artistsArray[i];
		const { name: artistsName, years } = currentArtist;
		const [birthYear, deathYear] = years.split(' - ');
		...
		// The actual logic for filtering the artists
		if (bornIn1900 && diedIn1900) {
			filteredArtistsArray.push(artistsName);
		}
	}
	return filteredArtistsArray;
}
